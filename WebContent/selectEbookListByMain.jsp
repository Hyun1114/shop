...
<body>
	...
	<!-- start :  adminMenu include -->
		...
		<h1>전자책 관리</h1>
			<form action="<%=request.getContextPath()%>/selectEbookListByMain.jsp">
...
<%
			...
			//시작 버튼
%>
			<a class="btn btn-danger" href="<%=request.getContextPath()%>/selectEbookListByMain.jsp?currentPage=1">처음으로</a>
<%
		
			// 이전 버튼
				if(startPage > nowPage){
%>
				<a class="btn btn-success" href="<%=request.getContextPath()%>/selectEbookListByMain.jsp?currentPage=<%=startPage-nowPage%>&categoryName=<%=categoryName%>">이전</a>
<%
			}
			...
			for(int i=startPage; i<=endPage; i++) {
				if(endPage < lastPage){
%>
					<a class="btn btn-success" href="<%=request.getContextPath()%>/selectEbookListByMain.jsp?currentPage=<%=i%>&categoryName=<%=categoryName%>"><%=i%></a>
<%
...
<%	
			...
			if(endPage < lastPage){
%>
			<a class="btn btn-success" href="<%=request.getContextPath()%>/selectEbookListByMain.jsp?currentPage=<%=startPage+nowPage%>&categoryName=<%=categoryName%>">다음</a>
<%
			}
%>
			<a class="btn btn-danger" href="<%=request.getContextPath()%>/selectEbookListByMain.jsp?currentPage=<%=lastPage%>&categoryName=<%=categoryName%>">끝으로</a>
<%
%>
		</div>
	</div>
