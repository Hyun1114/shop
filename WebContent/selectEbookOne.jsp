...
<%
			...
			//시작 버튼
%>
			<a class="btn btn-danger" href="<%=request.getContextPath()%>/selectEbookOne.jsp?currentPage=1&ebookNo=<%=ebookNo%>">처음으로</a>
<%
		
			// 이전 버튼
				if(startPage > nowPage){
%>
				<a class="btn btn-success" href="<%=request.getContextPath()%>/selectEbookOne.jsp?currentPage=<%=startPage-nowPage%>&ebookNo=<%=ebookNo%>">이전</a>
<%
			}
			...
			for(int i=startPage; i<=endPage; i++) {
				if(endPage < lastPage){
%>
					<a class="btn btn-success" href="<%=request.getContextPath()%>/selectEbookOne.jsp?currentPage=<%=startPage-nowPage%>&ebookNo=<%=ebookNo%>"><%=i%></a>
<%
...
<%	
			...
			if(endPage < lastPage){
%>
			<a class="btn btn-success" href="<%=request.getContextPath()%>/selectEbookOne.jsp?currentPage=<%=startPage-nowPage%>&ebookNo=<%=ebookNo%>">다음</a>
<%
			}
%>
			<a class="btn btn-danger" href="<%=request.getContextPath()%>/selectEbookOne.jsp?currentPage=<%=startPage-nowPage%>&ebookNo=<%=ebookNo%>">끝으로</a>
<%
%>
		</div>
		</div>
